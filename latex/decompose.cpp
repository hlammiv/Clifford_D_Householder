...
static void findClosestLatticePoints(
    ...
    vector<pair<ringZ9, double>>& results
) {
    int f_pow = 1;
    for (int i = 0; i < f; ++i) f_pow *= 3;
    int norm_target = f_pow * f_pow;

    // Bound on quad(a): q(a) ≤ 2·3^{2f} is needed for |a|² ~ 3^{2f}.
    // Use wider bound to catch good approximations.
    int A = 4 * norm_target;
    ...
    double dist_cutoff_sq = max_phase_error * max_phase_error * 4;

    // Use the SAME change-of-variables as entryEnumeration:
    //   q(a) = 3·a3² + 3·a4² + 3·a5² + b0² + b1² + b2²
    // where a0 = (b0+a3)/2, a1 = (b1+a4)/2, a2 = (b2+a5)/2
    // (requires b0 ≡ a3 mod 2, etc.)
    // This makes the form diagonal → tight bounds on each loop.

    int max_a3 = (int)ceil(sqrt(A / 3.0));

    // Thread-local result accumulation
    int n_threads = omp_get_max_threads();
    vector<vector<pair<ringZ9, double>>> tl_results(n_threads);

    #pragma omp parallel for schedule(dynamic) shared(tl_results)
    for (int a3 = -max_a3; a3 <= max_a3; ++a3) {
        int tid = omp_get_thread_num();
        auto& local_results = tl_results[tid];

        int budget_a3 = A - 3*a3*a3;
        if (budget_a3 < 0) continue;

        int max_a4 = (int)ceil(sqrt(budget_a3 / 3.0));
        for (int a4 = -max_a4; a4 <= max_a4; ++a4) {
            int budget_a4 = budget_a3 - 3*a4*a4;
            if (budget_a4 < 0) continue;

            int max_a5 = (int)ceil(sqrt(budget_a4 / 3.0));
            for (int a5 = -max_a5; a5 <= max_a5; ++a5) {
                int budget_a5 = budget_a4 - 3*a5*a5;
                if (budget_a5 < 0) continue;

                int max_b = (int)ceil(sqrt((double)budget_a5));
                for (int b0 = -max_b; b0 <= max_b; ++b0) {
                    if ((b0 + a3) % 2 != 0) continue;
                    int budget_b0 = budget_a5 - b0*b0;
                    if (budget_b0 < 0) continue;
                    int a0 = (b0 + a3) / 2;

                    int max_b1 = (int)ceil(sqrt((double)budget_b0));
                    for (int b1 = -max_b1; b1 <= max_b1; ++b1) {
                        if ((b1 + a4) % 2 != 0) continue;
                        int budget_b1 = budget_b0 - b1*b1;
                        if (budget_b1 < 0) continue;
                        int a1 = (b1 + a4) / 2;

                        int max_b2 = (int)ceil(sqrt((double)budget_b1));
                        for (int b2 = -max_b2; b2 <= max_b2; ++b2) {
                            ...
                            if (dist_sq > dist_cutoff_sq) continue;
                            ...
                            ringZ9 elem(arr);
                            local_results.push_back({elem, sqrt(dist_sq)});
                        }
                    }
                }
            }
    ...
    for (int t = 0; t < n_threads; ++t)
        results.insert(results.end(), tl_results[t].begin(), tl_results[t].end());
}
